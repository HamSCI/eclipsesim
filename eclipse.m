function eclipse(job_id, make_plot, use_tec, job_path, out_path, plot_path, tec_file)
%%----------------------------
...
%%----------------------------
    ...
    if use_tec == 1
        disp('Loading GPS-TEC data...');
    
        tec = h5read(tec_file, 'Data/Table Layout');
    
        interpolator = scatteredInterpolant(double(tec.gdlat(:)), ...
                                            double(tec.glon(:)), ...
                                            datenum(ones(1, length(tec))*2018, ...
                                                    ones(1, length(tec))*10, ...
                                                    ones(1, length(tec))*7, ...
                                                    data.hour, ...
                                                    data.min, ...
                                                    ones(1, lenght(tec))*0), ...
    ...
    out_file  = fopen(strcat(OUT_PATH, 'simulated_', '_', timestamp_str, '.csv'), 'w');
    ...
    while ~feof(job_file)
        ...
        out_all_fname   = strcat(OUT_PATH, 'all_',rx_call,'_',tx_call,'_','_', timestamp_str, '.csv');    

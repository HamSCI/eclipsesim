...
function [axis_handle, ray_handle, best_ray_handle] = ...
        plot_ray_iono_slice_custom(iono_grid, ...
      start_range, end_range, range_inc, start_height, end_height, ...
      height_inc, ray, best_ray)
...
for idx = 1:length(ray)
  ...
  if ~isempty(ray(idx).gndrng)
    ...
    try
      h = plot(ray_X, ray_Y, 'color', [1 1 0.99], 'linewidth', 1);
